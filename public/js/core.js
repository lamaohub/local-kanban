...
const ICONS = {
  ...
  doc: 'M9.2 1.8H4.6a1 1 0 0 0-1 1v10.4a1 1 0 0 0 1 1h6.8a1 1 0 0 0 1-1V4.6L9.2 1.8ZM9.2 1.8v2.8h3.2M6 8.2h4M6 10.8h4',
  eye: 'M1.8 8S4.2 3.8 8 3.8 14.2 8 14.2 8 11.8 12.2 8 12.2 1.8 8 1.8 8Z',
};
const ICON_EXTRA = {
  ...
  eye: '<circle cx="8" cy="8" r="1.7" fill="none" stroke="currentColor" stroke-width="1.3"/>',
...
const I18N_RU = {
  ...
  'a skill with this name already exists': 'скилл с таким именем уже есть',
  ...
  'ask the npm registry again': 'спросить реестр npm заново',
  'at': 'от',
  ...
  'cannot check — the npm registry did not answer': 'не удалось проверить — реестр npm не ответил',
  'Card actions': 'Управление карточкой',
  'change status': 'сменить статус',
  'changed — not written to the file yet': 'изменено — в файл ещё не записано',
  'Chaos': 'Хаос',
  'Check': 'Проверить',
  ...
  'Delete the skill': 'Удалить скилл',
  ...
  'Deploy skill': 'Деплой-скилл',
  'Deploy skills': 'Деплой-скиллы',
  'Deploy skills of the projects': 'Деплой-скиллы проектов',
  'Deploying': 'Деплою',
  ...
  'Fri': 'Пт',
  'free for everyone, nothing switched off — if it earns you money, $12 a year per person keeps it maintained': 'бесплатна для всех, ничего не отключено — если она зарабатывает тебе деньги, $12 в год за человека помогают её поддерживать',
  'from': 'от',
  ...
  'Install the update': 'Установить обновление',
  'installing…': 'ставлю обновление…',
  ...
  'linked to:': 'связано:',
  'Load the shared version': 'Загрузить общую версию',
  'loaded from GitHub': 'загружено с GitHub',
  'loaded from the installed package': 'загружено из установленного пакета',
  'Local': 'Локальные',
  ...
  'matches the registry': 'совпадает с реестром',
  ...
  'name: latin letters, digits, hyphen': 'имя: латиница, цифры, дефис',
  'name: latin letters, digits, hyphen, underscore only': 'имя: только латиница, цифры, дефис и подчёркивание',
  'needs attention': 'требует внимания',
  'Needs the gh CLI, signed in with the project scope:': 'Нужен gh CLI, авторизованный со scope project:',
  'new': 'новая',
  ...
  'no file': 'файла нет',
  'no folders inside': 'внутри нет папок',
  'no notes for this version': 'для этой версии заметок нет',
  'no such folder on this computer': 'такой папки на этом компьютере нет',
  'not deleted': 'не удалено',
  'not installed': 'не установлен',
  'not installed yet': 'ещё не установлена',
  'not set': 'не задан',
  'not set — no deploy': 'не задан — деплоя нет',
  'not there': 'нет файла',
  'nothing is written to disk until you save': 'на диск ничего не записано, пока не сохранишь',
  'nothing to save': 'нечего сохранять',
  'One repository for the whole board, not one per project. Create it yourself first, and make it private: task titles, descriptions and comments are copied there in full. This is the only thing on this board that leaves your machine.': 'Репозиторий ОДИН на всю доску, а не по одному на проект. Создай его заранее сам и сделай приватным: заголовки, описания и комментарии задач уезжают туда целиком. Это единственное, что покидает твою машину.',
  'Other Claude skills': 'Прочие скиллы Claude',
  'Overwrite': 'Перезаписать',
  'Overwrite this file?': 'Перезаписать этот файл?',
  'own skill': 'свой скилл',
  'Paths & deploy': 'Пути и деплой',
  'Project folder': 'Папка проекта',
  'restarting…': 'перезапускаюсь…',
  'Revert': 'Вернуть как в файле',
  's': 'с',
  'Save to the file': 'Сохранить в файл',
  'saved ✓': 'сохранено ✓',
  'saved ✓ · the previous version is kept as': 'сохранено ✓ · прежняя версия осталась как',
  'shared': 'общий',
  'shared deploy skill': 'общий деплой-скилл',
  'shared skill': 'общий скилл',
  'Skill file': 'Файл скилла',
  'Skills': 'Скиллы',
  'Skills are the instructions Claude reads. They live outside the board, in ~/.claude/skills, and the board only shows and updates them.': 'Скиллы — инструкции, по которым работает Claude. Они лежат вне доски, в ~/.claude/skills; доска их только показывает и обновляет.',
  'The board': 'Доска',
  'The board can mirror its tasks to GitHub: every task becomes an issue, and each project gets its own Projects v2 board. Handy when someone else needs to see the work, or when you want the history somewhere besides this machine.': 'Доска умеет зеркалить задачи в GitHub: каждая задача становится issue, а у каждого проекта заводится свой Projects v2. Пригождается, когда работу должен видеть кто-то ещё или когда историю хочется хранить не только на этой машине.',
  'the board has no shared version of this skill — it is your own': 'у доски нет общей версии этого скилла — он ваш собственный',
  'the field above takes a skill name — “deploy” is the shared one': 'в поле выше — имя скилла; «deploy» это общий',
  'The mirror is one-way: the board writes to GitHub and never reads back. Turning it off is emptying these fields.': 'Зеркало одностороннее: доска пишет в GitHub и обратно ничего не читает. Выключить — очистить эти поля.',
  'The project has no local folder — set one above and the notes will show up here.': 'У проекта нет локальной папки — укажи её выше, и заметки появятся здесь.',
  'The project notes and the deploy skill — Claude opens them before touching the code. Paths are absolute: copy one and paste it into the chat.': 'Заметки проекта и деплой-скилл — Claude открывает их до того, как трогать код. Пути полные: скопируй и вставь в чат.',
  'the skill Claude follows to ship this project; “deploy” is the shared one. A new one is created in Settings → Skills.': 'каким скиллом Claude выкатывает этот проект; «deploy» — общий. Свой заводится в «Настройки → Скиллы».',
  ...
  'the folder is there, but the board cannot read it — check its permissions': 'папка есть, но доска не может её прочитать — посмотри права доступа',
  ...
  'could not check the folder': 'не удалось проверить папку',
  ...
  'where the code lies on this computer — Claude edits and commits it here': 'где код лежит на этом компьютере — здесь Claude его правит и коммитит',
  'restarted on the server after a deploy; empty means there is nothing to restart': 'что перезапустить на сервере после выкатки; пусто — перезапускать нечего',
  'opened after a deploy to check the site answers': 'открывается после выкатки, чтобы убедиться, что сайт отвечает',
  'The same in the README, in more detail': 'То же самое в README, подробнее',
  'the board is not allowed to do this — the command is the same one it just tried': 'у доски нет на это прав — команда та же, что она только что пробовала',
  'Your tasks are not touched: they live in the data directory, apart from the code.': 'Задачи не пострадают: они лежат в папке данных, отдельно от кода.',
  'Afterwards refresh the skills Claude reads:': 'После обновления освежи скиллы, которые читает Claude:',
  'and refresh the skills:': 'и освежи скиллы:',
  'this is a link — only the link is removed, the file it points to stays': 'это ссылка — снимется только она, файл по ту сторону останется',
  'this is a symlink — saving rewrites the file it points to:': 'это ссылка — сохранение перезапишет файл, на который она ведёт:',
  'This project is gone': 'Этого проекта больше нет',
  'this skill was already on disk — added it to the list': 'такой скилл уже лежал на диске — добавил его в список',
  'too many folders to show them all': 'папок слишком много, показаны не все',
  'Update the board now? It will restart.': 'Обновить доску сейчас? Она перезапустится.',
  ...
  'To do': 'Сделать',
  'this path is not a folder': 'по этому пути не папка',
  'Today': 'Сегодня',
  ...
  'updated — start the board again to pick it up': 'обновлено — запусти доску заново, чтобы она подхватила новую версию',
  'updated, but the board did not come back — start it yourself': 'обновлено, но доска не поднялась — запусти её сам',
  'updating…': 'обновляю…',
  'Updating…': 'Обновляю…',
  'Upload': 'Загрузить',
  'Use this folder': 'Выбрать эту папку',
  'used by': 'используют',
  'Using the board at work': 'Пользуешься доской в работе',
  'version': 'версия',
  'waiting for the board to come back…': 'жду, пока доска поднимется…',
  ...
  'What Claude reads before working': 'Что Claude читает перед работой',
  ...
  'Without that scope an issue is still created, but the card never lands on the Projects board.': 'Без этого scope issue создастся, а карточка в Projects так и не появится.',
  'Work time': 'Время в работе',
  'working columns are moved by Claude only': 'рабочие колонки двигает только Claude',
  'Year': 'Год',
  'Years': 'Годы',
  'Yesterday': 'Вчера',
  'your own deploy skill is created and edited in Settings → Skills': 'свой деплой-скилл создаётся и правится в «Настройках → Скиллы»',
...
export const api = async (method, path, body, opts = {}) => {
  ...
  if (!res.ok) {
    ...
    throw apiError(data.error || res.status, { status: res.status, body: data });
  }
  return data;
};
